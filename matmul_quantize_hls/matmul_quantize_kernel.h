...
/*
    matmul_quantize_kernelはベクトル量子化をサポートします．
...
using W_INDEX_TYPE = ap_uint<GROUP_BITS>;
using W_VALUE_TYPE = float;
using CB_INTERNAL_TYPE= hls::vector<W_INTERNAL_TYPE, VECTOR_DIM>;
using CB_IO_TYPE = hls::vector<W_VALUE_TYPE, VECTOR_DIM>;

using X_INTERNAL_TYPE = ap_fixed<32,20>;
using Y_INTERNAL_TYPE = ap_fixed<48,25>;
...
auto constexpr ELEMENTS_BLOCK_W = BITWIDTH / (GROUP_BITS / VECTOR_DIM);
...
using BLOCK_W_INDEX = hls::vector<W_INDEX_TYPE, ELEMENTS_BLOCK_W / VECTOR_DIM>;
using BLOCK_W_VALUE = hls::vector<W_VALUE_TYPE, ELEMENTS_BLOCK_W>;
using BLOCK_W_PACKED   = ap_uint<ELEMENTS_BLOCK_W * GROUP_BITS / VECTOR_DIM>;
